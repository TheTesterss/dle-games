...
import { findPokemonByName, getRandomPokemon, IPokemon } from "../services/pokemon.service";
...
interface Room {
    ...
    firstSameFamily: any;
    mostInfoPlayer: any;
    forfeitVotes: Set<string>;
    disconnectTimeouts: Map<string, NodeJS.Timeout>;
    timeout?: NodeJS.Timeout;
    globalTimeout?: NodeJS.Timeout;
    playerTimeouts?: Map<string, NodeJS.Timeout>;
    chat: ChatMessage[];
    chatPinnedId?: string | null;
    logs: string[];
}
...
const userActiveGames = new Map<string, Set<string>>();

const DEBUG = process.env.DEBUG === 'true';

const logDebug = (...args: any[]) => {
    if (DEBUG) console.log('[POKEMON_DEBUG]', ...args);
};
...
const resolveTier = async (userId?: string | null) => {
    if (!userId) return null;
    try {
        const user = await Account.findById(userId, { premiumTier: 1, badges: 1 }).lean();
...
const capMaxPlayers = (tier: string | null, desired?: number) => {
    const maxAllowed = tier === "games_plus" ? 10 : tier === "games_one" ? 7 : 4;
    if (!desired || Number.isNaN(desired)) return maxAllowed;
    return Math.max(1, Math.min(maxAllowed, Math.floor(desired)));
};

const sanitizeAttributes = (attrs: any[] = [], tier: string | null) => {
    if (!Array.isArray(attrs)) return [];
    if (tier) return attrs;
    return attrs.filter((a) => !PREMIUM_ATTRS.has(a));
};

const sanitizeGenerations = (gens: any[] = [], tier: string | null) => {
    if (!Array.isArray(gens) || gens.length === 0) return tier ? [...FREE_GENERATIONS, ...PREMIUM_GENERATIONS] : FREE_GENERATIONS;
...
const compareNumeric = (value: number, target: number) => {
    if (value === target) return { state: "equal" };
    const diff = Math.abs(value - target);
    const ratio = target ? diff / target : diff;
    if (ratio <= 0.1) return { state: "close", direction: value > target ? "down" : "up" };
    return { state: value > target ? "down" : "up" };
};

const compareString = (value: string | null | undefined, target: string | null | undefined) => {
    if (!value || !target) return { state: "unknown" };
    if (value === target) return { state: "equal" };
    return { state: "different" };
};

const compareType1 = (guess: IPokemon, target: IPokemon) => compareString(guess?.type1, target?.type1);

const compareType2 = (guess: IPokemon, target: IPokemon) => {
    if (!target?.type2) {
        return guess?.type2 ? { state: "different" } : { state: "equal" };
    }
    if (!guess?.type2) return { state: "different" };
    return compareString(guess?.type2, target?.type2);
};

const compareColors = (guessColors: string[] = [], targetColors: string[] = []) => {
    if (!targetColors.length && !guessColors.length) return { state: "equal" };
    if (!targetColors.length && guessColors.length) return { state: "different" };
    if (!guessColors.length) return { state: "unknown" };
    const match = guessColors.filter((c) => targetColors.includes(c));
    if (match.length === 0) return { state: "different" };
    if (match.length === targetColors.length && guessColors.length === targetColors.length) return { state: "equal" };
    return { state: "partial" };
};

const comparePokemon = (guess: IPokemon, target: IPokemon, attributes: string[]) => {
    const result: any = {};
    if (!guess || !target) return result;
    attributes.forEach((attr) => {
        switch (attr) {
            case "weight": result.weight = compareNumeric(guess.weight, target.weight); break;
            case "size": result.size = compareNumeric(guess.size, target.size); break;
            case "generation": result.generation = compareNumeric(guess.generation, target.generation); break;
            case "index": result.index = compareNumeric(guess.index, target.index); break;
            case "types":
                result.type1 = compareType1(guess, target);
                result.type2 = compareType2(guess, target);
                break;
            case "colors": result.colors = compareColors(guess.colors || [], target.colors || []); break;
            case "evolution": result.evolution = compareNumeric(guess.evolutionStage, target.evolutionStage); break;
            case "legendary": result.legendary = compareString(String(guess.legendary), String(target.legendary)); break;
            case "fabulous": result.fabulous = compareString(String(guess.fabulous), String(target.fabulous)); break;
            case "mega": result.mega = compareString(String(guess.hasMegaEvolution), String(target.hasMegaEvolution)); break;
            case "maxEvolution": result.maxEvolution = compareString(String(guess.isMaxEvolution), String(target.isMaxEvolution)); break;
            case "ashTeam": result.ashTeam = compareString(String(guess.hasBeenInAshTeam), String(target.hasBeenInAshTeam)); break;
            case "genderDiff": result.genderDiff = compareString(String(guess.hasGenderDifference), String(target.hasGenderDifference)); break;
            case "formSwitch": result.formSwitch = compareString(String(guess.isFormSwitchable), String(target.isFormSwitchable)); break;
            case "baseHappiness": result.baseHappiness = compareNumeric(guess.baseHappiness, target.baseHappiness); break;
            case "captureRate": result.captureRate = compareNumeric(guess.captureRate, target.captureRate); break;
            default: break;
        }
    });
    return result;
...
const computeMatchScore = (result: any) => {
    const values = Object.values(result) as any[];
    if (values.length === 0) return 0;
    let score = 0;
    values.forEach((item) => {
        if (item.state === "equal") score += 1;
        else if (item.state === "partial" || item.state === "close") score += 0.5;
    });
    return score / values.length;
};

const saveGameStats = async (room: Room, winner: Player | null) => {
    ...
    const promises = Array.from(room.players.values()).map(async (player) => {
        ...
        try {
            ...
            await Account.updateOne({ _id: player.userId }, update);
...
const emitRooms = (io: Server) => {
    io.emit("pokemon:rooms", serializeRooms());
};

const pushChat = (io: Server, room: Room, message: ChatMessage) => {
    room.chat.push(message);
    if (room.chat.length > 200) room.chat = room.chat.slice(-200);
    io.to(room.code).emit("pokemon:chat_message", message);
};

const pushLog = (room: Room, message: string) => {
    room.logs.push(message);
    if (room.logs.length > 200) room.logs = room.logs.slice(-200);
};

const endGame = (io: Server, code: string, reason: string) => {
    ...
    players.forEach(p => {
        if (p.userId && userActiveGames.has(p.userId)) {
            ...
            if (userActiveGames.get(p.userId)?.size === 0) userActiveGames.delete(p.userId);
        }
    });
...
export const registerPokemonSocket = (io: Server) => {
    io.on("connection", (socket: Socket) => {
        ...
        if (userId) {
            ...
            if (userActiveGames.has(userId)) {
                ...
                const activeRooms = activeCodes
                    .map(c => rooms.get(c))
                    .filter((r): r is Room => !!r && r.started);
        ...
        socket.on("pokemon:invite", async (payload: any, cb: Function) => {
            try {
                if (!userId) return cb?.({ ok: false, error: "Non autorise" });
                const toUserId = payload?.toUserId;
                if (!toUserId) return cb?.({ ok: false, error: "Cible invalide" });
        ...
        socket.on("pokemon:rooms", (cb: Function) => {
            cb?.(serializeRooms());
        });

        socket.on("pokemon:create", async (payload: any, cb: Function) => {
            try {
                ...
                if (payload.userId) {
                    if (!userActiveGames.has(payload.userId)) userActiveGames.set(payload.userId, new Set());
        ...
        socket.on("pokemon:join", (payload: any, cb: Function) => {
            ...
            if (room.started) return cb?.({ ok: false, error: "La partie a deja commence" });
        ...
        socket.on("pokemon:start", (payload: any, cb: Function) => {
            const code = payload.code?.toUpperCase();
            const room = rooms.get(code);
            if (!room) return cb?.({ ok: false, error: "Room introuvable" });
            if (room.hostId !== socket.id) return cb?.({ ok: false, error: "Seul l'hote peut demarrer" });
            ...
            if (mode === "multi_same") {
                ...
                if (raceMode === "same") {
                    room.target = getRandomPokemon(new Set(), generations);
                    room.players.forEach(p => p.target = room.target!);
                } else {
                    const used = new Set<string>();
                    room.players.forEach((player) => {
                        const target = getRandomPokemon(used, generations);
        ...
        socket.on("pokemon:guess", (payload: any, cb: Function) => {
            const code = payload.code?.toUpperCase();
            const room = rooms.get(code);
            if (!room) return cb?.({ ok: false, error: "Room introuvable" });
            ...
            if (player.forfeited) return cb?.({ ok: false, error: "Vous avez abandonne" });
            ...
            if (mode === "multi_individual") {
                ...
                if (current !== socket.id) return cb?.({ ok: false, error: "Ce n'est pas votre tour" });
            }

            const guess = findPokemonByName(payload.guess);
            if (!guess) return cb?.({ ok: false, error: "Pokemon introuvable" });
            ...
            player.infoScore += Object.values(result).filter((r: any) => r.state === "equal" || r.state === "partial" || r.state === "close").length;
            ...
            if (allDone) {
                if (mode === "multi_individual") {
                    ...
                    if (!isCorrect) endGame(io, code, "attempts");
                    else endGame(io, code, "win");
                }
            }
        ...
        socket.on("pokemon:leave", (payload: any, cb: Function) => {
            const code = payload?.code?.toUpperCase();
            const room = rooms.get(code);
            if (!room) return cb?.({ ok: false, error: "Room introuvable" });
        ...
        socket.on("pokemon:chat", (payload: any, cb: Function) => {
            const code = payload?.code?.toUpperCase();
            const room = rooms.get(code);
            if (!room) return cb?.({ ok: false, error: "Room introuvable" });
            const player = room.players.get(socket.id);
            ...
            if (!content) return cb?.({ ok: false, error: "Message vide" });
        ...
        socket.on("pokemon:chat_pin", (payload: any, cb: Function) => {
            const code = payload?.code?.toUpperCase();
            const room = rooms.get(code);
            if (!room) return cb?.({ ok: false, error: "Room introuvable" });
            if (room.hostId !== socket.id) return cb?.({ ok: false, error: "Seul l'hôte peut épingler" });
            const messageId = String(payload?.messageId || "");
            if (!messageId) return cb?.({ ok: false, error: "Message invalide" });
        ...
        socket.on("pokemon:chat_react", (payload: any, cb: Function) => {
            const code = payload?.code?.toUpperCase();
            const room = rooms.get(code);
            if (!room) return cb?.({ ok: false, error: "Room introuvable" });
            const player = room.players.get(socket.id);
            ...
            if (!msg) return cb?.({ ok: false, error: "Message introuvable" });
            ...
            if (!existing) {
                ...
                else existing.users.push(userKey);
        ...
        socket.on("pokemon:watch", (payload: any, cb: Function) => {
            const code = payload?.code?.toUpperCase();
            ...
            if (!targetPlayer?.target) return cb?.({ ok: false, error: "Cible introuvable" });
        ...
        socket.on("pokemon:update_options", async (payload: any, cb: Function) => {
            const code = payload?.code?.toUpperCase();
            const room = rooms.get(code);
            if (!room) return cb?.({ ok: false, error: "Room introuvable" });
            if (room.hostId !== socket.id) return cb?.({ ok: false, error: "Seul le chef peut modifier" });
            if (room.started) return cb?.({ ok: false, error: "Partie deja lancee" });

            const tier = await resolveTier(room.hostUserId || payload?.userId || userId);
            const options = payload?.options || {};
            const next = { ...room.options };
            if (typeof options.mode === "string") next.mode = options.mode;
            if (typeof options.timeLimitSec === "number") next.timeLimitSec = options.timeLimitSec;
            if (typeof options.globalTimeSec === "number") next.globalTimeSec = options.globalTimeSec;
            if (typeof options.maxAttempts === "number") next.maxAttempts = options.maxAttempts;
            if (typeof options.isPrivate === "boolean") next.isPrivate = options.isPrivate;
            if (Array.isArray(options.attributes)) next.attributes = sanitizeAttributes(options.attributes, tier);
            if (Array.isArray(options.generations)) next.generations = sanitizeGenerations(options.generations, tier);
        ...
        socket.on("pokemon:check_active", (payload: any) => {
        ...
        socket.on("pokemon:host_transfer", (payload: any, cb: Function) => {
            const code = payload?.code?.toUpperCase();
            const targetId = payload?.targetId;
            const room = rooms.get(code);
            if (!room) return cb?.({ ok: false, error: "Room introuvable" });
            if (room.hostId !== socket.id) return cb?.({ ok: false, error: "Seul le chef peut transferer" });
            const target = room.players.get(targetId);
            if (!target) return cb?.({ ok: false, error: "Joueur introuvable" });
        ...
        socket.on("pokemon:disband", (payload: any, cb: Function) => {
            const code = payload?.code?.toUpperCase();
            const room = rooms.get(code);
            if (!room) return cb?.({ ok: false, error: "Room introuvable" });
            if (room.hostId !== socket.id) return cb?.({ ok: false, error: "Seul le chef peut demanteler" });
            endGame(io, code, "disbanded");
            io.to(code).emit("pokemon:ended", { reason: "disbanded" });
            cb?.({ ok: true });
        });

        socket.on("pokemon:forfeit", (payload: any, cb: Function) => {
            const code = payload?.code?.toUpperCase();
            const room = rooms.get(code);
            if (!room) return cb?.({ ok: false, error: "Room introuvable" });
            const player = room.players.get(socket.id);
            ...
            if (mode === "solo") {
                ...
                const allDone = Array.from(room.players.values()).every((p) => p.finishedAt || p.forfeited);
                if (allDone) endGame(io, code, "forfeit");
            }
            cb?.({ ok: true });
        });
                    ...
                    const timeout = setTimeout(() => {
                        ...
                        if (room.players.has(player.id)) {
                            if (room.started) {
                                ...
                                const allDone = Array.from(room.players.values()).every((p) => p.finishedAt || p.forfeited);
                                if (allDone) endGame(io, code, "dropout");
                            } else {
